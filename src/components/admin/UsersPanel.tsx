...
const AVATARS = ['­ƒæñ','­ƒºÖ','­ƒºÖÔÇìÔÖÇ´©Å','­ƒªü','­ƒÉì','­ƒªà','­ƒªí','ÔÜí','­ƒîƒ','­ƒö«','­ƒÅ░','­ƒôÜ','­ƒº¬','­ƒªï','­ƒÉë','­ƒÄ»','­ƒøí´©Å','­ƒîè','­ƒöÑ','­ƒî┐','­ƒÆÄ','­ƒªè','­ƒÉ║','­ƒªë','­ƒÉØ','­ƒÉÖ','­ƒªä','­ƒÉº','­ƒÉ╗','­ƒÉ¼','­ƒªê','­ƒÉó','­ƒªç','­ƒî©','­ƒìÇ','­ƒîÖ','ÔÿÇ´©Å','­ƒîê','­ƒÄ▓','­ƒÄ¡','­ƒÜÇ','­ƒÄ©','­ƒÄ¿','­ƒÅå','­ƒÄ¬','­ƒº¼','­ƒö¼','­ƒÆ╗','­ƒÄ«','­ƒÄº','­ƒô▒','­ƒø©','­ƒîì','­ƒùí´©Å','­ƒÅ╣','­ƒº▓','­ƒöæ','­ƒùØ´©Å','­ƒÄ®','­ƒææ']
...
const emptyForm: UserForm = { name: '', username: '', password: '', email: '', company: 'ALTEN', role_label: '', avatar: '­ƒæñ', color: '#007AFF', is_superuser: false, calendario_id: '', cost_rates: [], hire_date: '', contract_type: 'indefinido', convenio: '', projects: [], responsable_id: '', vacation_carryover: 0 }
...
export function UsersPanel() {
  ...
  const calNameOf = (m: Member) => getCal(m)?.name || 'ÔÇö'
  ...
  // Bulk fichaje by date range ÔÇö for each selected person, for each business day, insert hours according to their calendar
  const applyBulkFichaje = async () => {
    ...
    setBulkFichajeSaving(false); setBulkFichajeResult(`${totalDays} d├¡as fichados para ${selected.size} personas${skipped > 0 ? ` (${skipped} ya fichados, omitidos)` : ''}`)
    setSelected(new Set())
  }

  const downloadTemplate = async () => {
    const XLSX = await import('xlsx')
    const ws = XLSX.utils.aoa_to_sheet([['nombre*','email*','usuario','contrase├▒a','empresa','rol','contrato','fecha_alta','coste_hora','calendario','responsable_email','vacaciones_pendientes','telefono'],['Juan P├®rez','juan@empresa.com','jperez','revelio2026','ALTEN','Consultor','indefinido','2024-01-15','25','Madrid 2026','jefe@empresa.com','3','666123456']])
  ...
  const handleImportExcel = async (file: File) => {
    ...
    try {
      ...
      const ws = wb.Sheets[wb.SheetNames[0]!]; if (!ws) throw new Error('Hoja vac├¡a')
      ...
      for (const row of rows) {
        ...
        const { error: tmErr } = await supabase.from('team_members').insert({ id, name, username: email.split('@')[0] || name.toLowerCase().replace(/\s+/g,'.'), email, avatar: AVATARS[Math.floor(Math.random()*AVATARS.length)]||'­ƒæñ', color: COLORS[Math.floor(Math.random()*COLORS.length)]||'#007AFF', company: String(row['empresa']||'ALTEN'), role_label: String(row['rol']||''), is_superuser: false, rooms: [], cost_rates: [], preferences: {} })
        ...
        if (email) {
          ...
          const { error: aE } = await supabase.rpc('create_auth_user', { user_email: email, user_password: String(row['contrase├▒a']||'revelio2026'), user_id: id })
  ...
  const openEdit = (m: Member) => {
    ...
    setForm({ name: m.name, username: m.username||'', password: '', email: m.email||'', company: m.company||'ALTEN', role_label: m.role_label||'', avatar: m.avatar||'­ƒæñ', color: m.color||'#007AFF', is_superuser: m.is_superuser||false, calendario_id: (rx(m).calendario_id as string)||'', cost_rates: costRates, hire_date: (rx(m).hire_date as string)||'', contract_type: (rx(m).contract_type as string)||'indefinido', convenio: (rx(m).convenio as string)||'', projects, responsable_id: (rx(m).responsable_id as string)||'', vacation_carryover: Number(rx(m).vacation_carryover)||0 })
  ...
  return (
    <div className="max-w-[1200px]">
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div><h2 className="text-lg font-semibold text-revelio-text dark:text-revelio-dark-text">Equipo</h2><p className="text-xs text-revelio-subtle dark:text-revelio-dark-subtle">{members.length} personas{hasSelection ? ` ┬À ${selected.size} seleccionados` : ''}</p></div>
      ...
      {hasSelection && viewMode === 'general' && (
        <div className="rounded-lg bg-revelio-blue/5 border border-revelio-blue/20 px-4 py-2.5 mb-3 flex items-center gap-3 flex-wrap">
          ...
          <select value={bulkAction || ''} onChange={e => { setBulkAction(e.target.value || null); setBulkValue('') }} className="rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-[10px] outline-none bg-white dark:bg-revelio-dark-bg dark:text-revelio-dark-text">
            <option value="">Acci├│n masiva...</option><option value="company">Cambiar empresa</option><option value="role">Cambiar rol</option><option value="calendario">Asignar calendario</option><option value="project">Asignar proyecto</option><option value="status">Cambiar estado</option>
          </select>
      ...
      {/* BULK FICHAJE BAR ÔÇö Jornada (date range) */}
      {hasSelection && viewMode === 'jornada' && (
        <div className="rounded-lg bg-revelio-orange/5 border border-revelio-orange/20 px-4 py-2.5 mb-3 flex items-center gap-3 flex-wrap">
          <span className="text-[10px] font-semibold text-revelio-orange">{selected.size} seleccionados ÔÇö Fichaje masivo</span>
          ...
          <span className="text-[8px] text-revelio-subtle">Horas seg├║n convenio de cada persona</span>
      ...
      <div className="rounded-card border border-revelio-border dark:border-revelio-dark-border bg-white dark:bg-revelio-dark-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>{filtered.map((m, i) => { const ded = memberDedToday(m); const inter = Math.max(0, 1-ded); const showIC = inter > 0; const projs = memberProjects(m); const status = (rx(m).contract_type as string) === 'inactive' ? 'inactive' : 'active'; const sel = selected.has(m.id); return (
          <tr key={m.id} className={`${sel ? 'bg-revelio-blue/5' : i%2 ? 'bg-revelio-bg/20 dark:bg-revelio-dark-border/10' : ''} hover:bg-revelio-blue/5`}>
            ...
            <td className={`${td} text-center`}><span className="text-base" style={{color:m.color}}>{m.avatar||'­ƒæñ'}</span></td>
            <td className={`${td} font-semibold text-revelio-blue cursor-pointer whitespace-nowrap`} onClick={() => openEdit(m)}>{m.name}</td>
            <td className={`${td} text-center text-revelio-subtle dark:text-revelio-dark-subtle`}>{m.username||'ÔÇö'}</td>
            <td className={`${td} text-center text-revelio-subtle dark:text-revelio-dark-subtle text-[9px]`}>{m.email||'ÔÇö'}</td>
            <td className={`${td} text-center dark:text-revelio-dark-text`}>{m.company||'ÔÇö'}</td>
            <td className={`${td} text-center`}>{m.role_label ? <span className="text-[8px] font-semibold px-2 py-0.5 rounded" style={{background:(ROLE_COLORS[m.role_label]||'#5856D6')+'18',color:ROLE_COLORS[m.role_label]||'#5856D6'}}>{m.role_label}</span> : 'ÔÇö'}</td>
            <td className={`${td} text-center max-w-[140px]`}><div className="flex gap-0.5 flex-wrap justify-center">{projs.length > 0 ? projs.map((p,pi) => <span key={pi} className="text-[7px] font-semibold px-1.5 py-0.5 rounded bg-revelio-blue/10 text-revelio-blue truncate max-w-[80px]">{p}</span>) : <span className="text-revelio-subtle">ÔÇö</span>}</div></td>
            <td className={`${td} text-center font-bold`} style={{color:ded>1?'#FF3B30':ded===1?'#34C759':ded>0?'#FF9500':'#8E8E93'}}>{Math.round(ded*100)}%</td>
            <td className={`${td} text-center font-semibold`} style={{color:showIC?'#FF9500':'#D1D1D6'}}>{showIC?`${Math.round(inter*100)}%`:'ÔÇö'}</td>
        ...
        <tbody>{filtered.map((m, i) => { const cal = getCal(m); const fichH = fichadasYTD(m); const expH2 = expectedH(m); const diff = Math.round((fichH-expH2)*10)/10; const ded = memberDedToday(m); const inter = Math.max(0,1-ded); const convH = cal?.convenio_hours || 0; const expYr = expectedHoursToDate(cal, yr, `${yr}-12-31`); const sel = selected.has(m.id); return (
          <tr key={m.id} className={`${sel ? 'bg-revelio-orange/5' : i%2?'bg-revelio-bg/20 dark:bg-revelio-dark-border/10':''} hover:bg-revelio-blue/5`}>
            ...
            <td className={`${td} text-center font-semibold dark:text-revelio-dark-text`}>{convH > 0 ? `${fmtN(convH)}h` : 'ÔÇö'}</td>
            <td className={`${td} text-center dark:text-revelio-dark-text`}>{cal ? `${fmtN(expYr)}h` : 'ÔÇö'}</td>
            ...
            <td className={`${td} text-center font-semibold`} style={{color:inter>0?'#FF9500':'#D1D1D6'}}>{inter>0?`${Math.round(inter*100)}%`:'ÔÇö'}</td>
            <td className={`${td} text-center`}><button onClick={() => openEdit(m)} className="w-5 h-5 rounded border border-revelio-border dark:border-revelio-dark-border flex items-center justify-center hover:bg-revelio-bg dark:hover:bg-revelio-dark-border"><Edit className="w-2.5 h-2.5 text-revelio-blue" /></button></td>
          </tr>) })}</tbody></>}

        {/* COSTES ÔÇö without Rol */}
        {viewMode === 'costes' && <><thead><tr className="bg-revelio-bg/50 dark:bg-revelio-dark-border/30"><th className={th}></th><th className={`${th} text-left`}>Persona</th><th className={th}>Coste/h</th><th className={th}>Coste anual</th><th className={th}>Coste IC/d├¡a</th><th className={th}>Venta proy.</th><th className={th}>Rentabilidad</th><th className={th}></th></tr></thead>
        <tbody>{filtered.map((m, i) => { const crRaw = rx(m).cost_rates; const costH = getCurrentRate(Array.isArray(crRaw)?crRaw as CostRate[]:[]); const cal = getCal(m); const convH = cal?.convenio_hours || 1800; const costAnual = Math.round(costH * convH); const costICDia = costH > 0 ? Math.round(costH * (cal?.daily_hours_lj || 8)) : 0; return (
          <tr key={m.id} className={`${i%2?'bg-revelio-bg/20 dark:bg-revelio-dark-border/10':''} hover:bg-revelio-blue/5`}>
            <td className={`${td} text-center`}><span className="text-base" style={{color:m.color}}>{m.avatar||'­ƒæñ'}</span></td>
            <td className={`${td} cursor-pointer whitespace-nowrap`} onClick={() => openEdit(m)}><p className="font-semibold text-revelio-blue">{m.name}</p><p className="text-[8px] text-revelio-subtle dark:text-revelio-dark-subtle">{m.company||'ÔÇö'} ┬À {m.role_label||'ÔÇö'}</p></td>
            <td className={`${td} text-center`}>{costH>0?<span className="font-bold text-revelio-green">{costH}Ôé¼/h</span>:<span className="text-revelio-subtle">ÔÇö</span>}</td>
            <td className={`${td} text-center font-semibold dark:text-revelio-dark-text`}>{costAnual>0?`${costAnual.toLocaleString('es-ES')}Ôé¼`:'ÔÇö'}</td>
            <td className={`${td} text-center`}>{costICDia>0?<span className="text-revelio-red font-semibold">{costICDia}Ôé¼/d├¡a</span>:'ÔÇö'}</td>
            <td className={`${td} text-center text-revelio-subtle`}>ÔÇö</td>
            <td className={`${td} text-center text-revelio-subtle`}>ÔÇö</td>
            <td className={`${td} text-center`}><button onClick={() => openEdit(m)} className="w-5 h-5 rounded border border-revelio-border dark:border-revelio-dark-border flex items-center justify-center hover:bg-revelio-bg dark:hover:bg-revelio-dark-border"><Edit className="w-2.5 h-2.5 text-revelio-blue" /></button></td>
          </tr>) })}</tbody></>}
      ...
      {modal && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}><div onClick={e => e.stopPropagation()} className="bg-white dark:bg-revelio-dark-card rounded-2xl max-w-lg w-full p-6 shadow-xl max-h-[85vh] overflow-y-auto">
        ...
        {tab === 'general' && (<div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3"><div><L>Email</L><I value={form.email} onChange={v => setForm({...form,email:v})} /></div><div><L>Contrase├▒a</L><input type="password" value={form.password} onChange={e => setForm({...form,password:e.target.value})} className="w-full rounded-lg border border-revelio-border dark:border-revelio-dark-border px-3 py-2 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" placeholder={modal==='edit'?'vac├¡o = mantener':''} /></div></div>
        ...
        {tab === 'costes' && (<div className="space-y-4"><div><div className="flex items-center justify-between mb-2"><L>Coste empresa (Ôé¼/hora)</L><button onClick={() => setForm({...form,cost_rates:[...form.cost_rates,{from:new Date().toISOString().slice(0,7),rate:0}]})} className="text-[9px] text-revelio-blue font-medium flex items-center gap-0.5"><Plus className="w-3 h-3" /> Periodo</button></div>
