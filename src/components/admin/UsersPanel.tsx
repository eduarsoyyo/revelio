    ...
    <div className="max-w-[1200px]">
      ...
      {modal && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}><div onClick={e => e.stopPropagation()} className="bg-white dark:bg-revelio-dark-card rounded-2xl max-w-lg w-full p-6 shadow-xl max-h-[85vh] overflow-y-auto">
        ...
        {tab === 'costes' && (<div className="space-y-4"><div><div className="flex items-center justify-between mb-2"><L>Coste empresa (€/hora)</L><button onClick={() => setForm({...form,cost_rates:[...form.cost_rates,{from:new Date().toISOString().slice(0,7),salary:0,multiplier:1.33}]})} className="text-[9px] text-revelio-blue font-medium flex items-center gap-0.5"><Plus className="w-3 h-3" /> Periodo</button></div>
          ...
          {form.cost_rates.map((cr,i) => { const cr2 = cr as CostRate & { salary?: number; multiplier?: number }; const cal = form.calendario_id ? calendarios.find(c => c.id === form.calendario_id) : null; const convH = cal?.convenio_hours || 1800; const sal = cr2.salary || 0; const mult = cr2.multiplier ?? 1.33; const costH = sal > 0 ? Math.round((sal * mult / convH) * 100) / 100 : (cr2.rate || 0); return (<div key={i} className="flex gap-2 items-center mb-2 bg-revelio-bg dark:bg-revelio-dark-border rounded-lg px-3 py-2"><div className="flex-1 grid grid-cols-5 gap-2"><div><label className="text-[8px] text-revelio-subtle">Desde</label><input type="month" value={cr.from} onChange={e=>{const n=[...form.cost_rates];n[i]={...n[i]!,from:e.target.value};setForm({...form,cost_rates:n})}} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-[10px] outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" /></div><div><label className="text-[8px] text-revelio-subtle">Hasta</label><input type="month" value={cr.to||''} onChange={e=>{const n=[...form.cost_rates];n[i]={...n[i]!,to:e.target.value||undefined};setForm({...form,cost_rates:n})}} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-[10px] outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" placeholder="actual" /></div><div><label className="text-[8px] text-revelio-subtle">Salario bruto anual (€)</label><input type="number" value={sal||''} onChange={e=>{const n=[...form.cost_rates];const ni:any={...n[i]!,salary:Number(e.target.value)};delete ni.rate;n[i]=ni;setForm({...form,cost_rates:n})}} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-[10px] outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text text-right font-bold" step={1000} placeholder="52000" /></div><div><label className="text-[8px] text-revelio-subtle">Multiplicador</label><input type="number" value={mult} onChange={e=>{const n=[...form.cost_rates];n[i]={...n[i]!,multiplier:Number(e.target.value)};setForm({...form,cost_rates:n})}} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-[10px] outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text text-right" step={0.01} placeholder="1.33" /></div><div><label className="text-[8px] text-revelio-subtle">Coste/h</label><div className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-[10px] bg-revelio-bg/30 dark:bg-revelio-dark-border/30 text-right font-bold text-revelio-green">{costH}€</div></div></div><button onClick={()=>setForm({...form,cost_rates:form.cost_rates.filter((_,j)=>j!==i)})} className="text-revelio-subtle hover:text-revelio-red"><X className="w-3 h-3" /></button></div>) })}
