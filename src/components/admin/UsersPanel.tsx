...
export function UsersPanel() {
  ...
  const th = 'px-2 py-2 text-[10px] font-bold text-revelio-subtle dark:text-revelio-dark-subtle uppercase whitespace-nowrap border-b-2 border-revelio-border dark:border-revelio-dark-border'
  const td = 'px-2 py-2 border-b border-revelio-border/50 dark:border-revelio-dark-border/50 text-xs'
  ...
  return (
    <div className="w-full">
      ...
      {hasSelection && viewMode === 'general' && (<div className="rounded-lg bg-revelio-blue/5 border border-revelio-blue/20 px-4 py-2.5 mb-3 flex items-center gap-3 flex-wrap">
        <span className="text-xs font-semibold text-revelio-blue">{selected.size} sel.</span>
        <select value={bulkAction||''} onChange={e => {setBulkAction(e.target.value||null);setBulkValue('')}} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none bg-white dark:bg-revelio-dark-bg dark:text-revelio-dark-text"><option value="">Acción masiva...</option><option value="company">Empresa</option><option value="role">Rol</option><option value="calendario">Calendario</option><option value="project">Proyecto</option><option value="status">Estado</option></select>
        {bulkAction==='company' && <input value={bulkValue} onChange={e=>setBulkValue(e.target.value)} placeholder="Empresa" className="rounded border border-revelio-border px-2 py-1 text-xs outline-none w-28 dark:bg-revelio-dark-bg dark:text-revelio-dark-text" />}
        {bulkAction==='role' && <select value={bulkValue} onChange={e=>setBulkValue(e.target.value)} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none bg-white dark:bg-revelio-dark-bg dark:text-revelio-dark-text"><option value="">Rol...</option>{roles.map(r=><option key={r} value={r}>{r}</option>)}</select>}
        {bulkAction==='calendario' && <select value={bulkValue} onChange={e=>setBulkValue(e.target.value)} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none bg-white dark:bg-revelio-dark-bg dark:text-revelio-dark-text"><option value="">Calendario...</option>{calendarios.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>}
        {bulkAction==='project' && <select value={bulkValue} onChange={e=>setBulkValue(e.target.value)} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none bg-white dark:bg-revelio-dark-bg dark:text-revelio-dark-text"><option value="">Proyecto...</option>{rooms.map(r=><option key={r.slug} value={r.slug}>{r.name}</option>)}</select>}
        {bulkAction==='status' && <select value={bulkValue} onChange={e=>setBulkValue(e.target.value)} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none bg-white dark:bg-revelio-dark-bg dark:text-revelio-dark-text"><option value="">Estado...</option><option value="indefinido">Activo</option><option value="inactive">Inactivo</option></select>}
        {bulkAction && bulkValue && <button onClick={applyBulk} disabled={saving} className="px-3 py-1 rounded bg-revelio-blue text-white text-xs font-semibold disabled:opacity-40"><Check className="w-3 h-3 inline mr-1" />Aplicar</button>}
        <button onClick={()=>setSelected(new Set())} className="text-xs text-revelio-subtle hover:underline ml-auto">Cancelar</button>
      </div>)}
      ...
      {hasSelection && viewMode === 'jornada' && (<div className="rounded-lg bg-revelio-orange/5 border border-revelio-orange/20 px-4 py-2.5 mb-3 flex items-center gap-3 flex-wrap">
        <span className="text-xs font-semibold text-revelio-orange">{selected.size} sel. — Fichaje masivo</span>
        <div className="flex items-center gap-1"><label className="text-[10px] text-revelio-subtle">Desde</label><input type="date" value={bulkFrom} onChange={e=>setBulkFrom(e.target.value)} max={today} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" /></div>
        <div className="flex items-center gap-1"><label className="text-[10px] text-revelio-subtle">Hasta</label><input type="date" value={bulkTo} onChange={e=>setBulkTo(e.target.value)} max={today} className="rounded border border-revelio-border px-2 py-1 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" /></div>
        <span className="text-[10px] text-revelio-subtle">Horas según convenio de cada persona</span>
        <button onClick={applyBulkFichaje} disabled={bulkFichajeSaving||!bulkFrom||!bulkTo} className="px-3 py-1 rounded bg-revelio-orange text-white text-xs font-semibold disabled:opacity-40">{bulkFichajeSaving?'Fichando...':'Fichar rango'}</button>
        <button onClick={()=>setSelected(new Set())} className="text-xs text-revelio-subtle hover:underline ml-auto">Cancelar</button>
      </div>)}
      ...
      <div className="rounded-card border border-revelio-border dark:border-revelio-dark-border bg-white dark:bg-revelio-dark-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>{filtered.map((m,i) => { const ded=memberDedToday(m); const inter=Math.max(0,1-ded); const showIC=inter>0; const projs=memberProjects(m); const status=(rx(m).contract_type as string)==='inactive'?'inactive':'active'; const sel=selected.has(m.id); return (
          <tr key={m.id} className={`${sel?'bg-revelio-blue/5':i%2?'bg-revelio-bg/20 dark:bg-revelio-dark-border/10':''} hover:bg-revelio-blue/5`}>
            ...
            <td className={`${td} text-center text-revelio-subtle dark:text-revelio-dark-subtle text-[11px]`}>{m.email||'—'}</td>
            <td className={`${td} text-center dark:text-revelio-dark-text`}>{m.company||'—'}</td>
            <td className={`${td} text-center`}>{m.role_label?<span className="text-[10px] font-semibold px-2 py-0.5 rounded" style={{background:(ROLE_COLORS[m.role_label]||'#5856D6')+'18',color:ROLE_COLORS[m.role_label]||'#5856D6'}}>{m.role_label}</span>:'—'}</td>
            <td className={`${td} text-center max-w-[140px]`}><div className="flex gap-0.5 flex-wrap justify-center">{projs.length>0?projs.map((p,pi)=><span key={pi} className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-revelio-blue/10 text-revelio-blue truncate max-w-[80px]">{p}</span>):<span className="text-revelio-subtle">—</span>}</div></td>
            ...
            <td className={`${td} text-center dark:text-revelio-dark-text text-[11px]`}>{calNameOf(m)}</td>
            ...
            <td className={`${td} text-center`}><span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${status==='active'?'bg-revelio-green/10 text-revelio-green':'bg-revelio-bg text-revelio-subtle'}`}>{status==='active'?'Activo':'Inactivo'}</span></td>
        ...
        <tbody>{filtered.map((m,i) => { const cal=getCal(m); const fichH=fichadasYTD(m); const expH2=expectedH(m); const diff=Math.round((fichH-expH2)*10)/10; const ded=memberDedToday(m); const inter=Math.max(0,1-ded); const convH=cal?.convenio_hours||0; const vacD=vacDaysApproved(absReqs,m.id,yr); const ausD=ausDaysApproved(absReqs,m.id,yr); const expYr=effectiveTheoreticalHoursYear(cal,yr,vacD,ausD); const sel=selected.has(m.id); return (
          <tr key={m.id} className={`${sel?'bg-revelio-orange/5':i%2?'bg-revelio-bg/20 dark:bg-revelio-dark-border/10':''} hover:bg-revelio-blue/5`}>
            ...
            <td className={`${td} text-center`}><span className="text-base" style={{color:m.color}}>{m.avatar||'👤'}</span></td>
            <td className={`${td} cursor-pointer whitespace-nowrap`} onClick={()=>openEdit(m)}><p className="font-semibold text-revelio-blue">{m.name}</p><p className="text-[10px] text-revelio-subtle dark:text-revelio-dark-subtle">{calNameOf(m)}</p></td>
        ...
        <tbody>{filtered.map((m,i) => {
          ...
          <tr key={m.id} className={`${i%2?'bg-revelio-bg/20 dark:bg-revelio-dark-border/10':''} hover:bg-revelio-blue/5`}>
            <td className={`${td} text-center`}><span className="text-base" style={{color:m.color}}>{m.avatar||'👤'}</span></td>
            <td className={`${td} cursor-pointer whitespace-nowrap`} onClick={()=>openEdit(m)}><p className="font-semibold text-revelio-blue">{m.name}</p><p className="text-[10px] text-revelio-subtle dark:text-revelio-dark-subtle">{m.company||'—'} · {m.role_label||'—'}</p></td>
            ...
            <td className={`${td} text-center`}>{totalProjCost>0?<div className="flex flex-col items-center gap-0.5">{projCosts.map((p,pi)=><span key={pi} className="text-[9px]"><span className="text-revelio-blue font-semibold">{p.name}</span> {fmtEur(p.cost)}</span>)}<span className="text-[10px] font-bold dark:text-revelio-dark-text border-t border-revelio-border/30 pt-0.5 mt-0.5">{fmtEur(totalProjCost)}</span></div>:'—'}</td>
            <td className={`${td} text-center`}><button onClick={()=>openEdit(m)} className="w-5 h-5 rounded border border-revelio-border dark:border-revelio-dark-border flex items-center justify-center hover:bg-revelio-bg dark:hover:bg-revelio-dark-border"><Edit className="w-2.5 h-2.5 text-revelio-blue" /></button></td>
          </tr>)})}</tbody></>}
      ...
      {modal && (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={()=>setModal(null)}><div onClick={e=>e.stopPropagation()} className="bg-white dark:bg-revelio-dark-card rounded-2xl max-w-lg w-full p-6 shadow-xl max-h-[85vh] overflow-y-auto">
        ...
        {tab === 'general' && (<div className="space-y-3">
          ...
          <div><L>Proyectos</L><div className="flex gap-1 flex-wrap mb-2">{rooms.map(r=>{const a=form.projects.some(p=>p.slug===r.slug);return <button key={r.slug} onClick={()=>toggleProject(r.slug)} className={`px-2 py-0.5 rounded text-[11px] font-semibold ${a?'bg-revelio-blue text-white':'bg-revelio-bg dark:bg-revelio-dark-border text-revelio-subtle dark:text-revelio-dark-subtle'}`}>{r.name}</button>})}</div>
            {form.projects.length>0 && <div className="space-y-1.5">{form.projects.map(pa=>{const rN=rooms.find(r=>r.slug===pa.slug)?.name||pa.slug;return(<div key={pa.slug} className="rounded-lg bg-revelio-bg dark:bg-revelio-dark-border px-3 py-2"><div className="flex items-center justify-between mb-1"><span className="text-xs font-semibold text-revelio-blue">{rN}</span><button onClick={()=>toggleProject(pa.slug)} className="text-revelio-subtle hover:text-revelio-red"><X className="w-3 h-3" /></button></div><div className="grid grid-cols-3 gap-2"><div><label className="text-[10px] text-revelio-subtle">Dedicación %</label><input type="number" value={pa.dedication} onChange={e=>updProject(pa.slug,'dedication',Number(e.target.value))} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-xs outline-none text-right dark:bg-revelio-dark-bg dark:text-revelio-dark-text" min={0} max={100} step={5} /></div><div><label className="text-[10px] text-revelio-subtle">Desde</label><input type="date" value={pa.from} onChange={e=>updProject(pa.slug,'from',e.target.value)} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-1.5 py-1 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" /></div><div><label className="text-[10px] text-revelio-subtle">Hasta</label><input type="date" value={pa.to} onChange={e=>updProject(pa.slug,'to',e.target.value)} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-1.5 py-1 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text" /></div></div></div>)})}</div>}
          </div>
        ...
        {tab === 'costes' && (<div className="space-y-4"><div>
          <div className="flex items-center justify-between mb-2"><L>Salario bruto anual con periodos</L><button onClick={()=>setForm({...form,cost_rates:[...form.cost_rates,{from:new Date().toISOString().slice(0,7),salary:0,multiplier:DEFAULT_SS_MULTIPLIER}]})} className="text-[11px] text-revelio-blue font-medium flex items-center gap-0.5"><Plus className="w-3 h-3" /> Periodo</button></div>
          <p className="text-[11px] text-revelio-subtle dark:text-revelio-dark-subtle mb-2">Cuando hay subida salarial, añade un nuevo periodo.</p>
          {form.cost_rates.length===0 && <p className="text-xs text-revelio-subtle bg-revelio-bg dark:bg-revelio-dark-border rounded-lg px-3 py-2">Sin salario definido.</p>}
          {form.cost_rates.map((cr,i) => {
            ...
            <div key={i} className="mb-3 bg-revelio-bg dark:bg-revelio-dark-border rounded-lg px-3 py-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div><label className="text-[10px] text-revelio-subtle">Salario bruto €/año</label><input type="number" value={cr.salary||''} onChange={e=>{const n=[...form.cost_rates];n[i]={...n[i]!,salary:Number(e.target.value)};setForm({...form,cost_rates:n})}} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text text-right font-bold" step={500} /></div>
                <div><label className="text-[10px] text-revelio-subtle">Multiplicador SS</label><input type="number" value={cr.multiplier} onChange={e=>{const n=[...form.cost_rates];n[i]={...n[i]!,multiplier:Number(e.target.value)};setForm({...form,cost_rates:n})}} className="w-full rounded border border-revelio-border dark:border-revelio-dark-border px-2 py-1 text-xs outline-none dark:bg-revelio-dark-bg dark:text-revelio-dark-text text-right" step={0.01} min={1} max={2} /></div>
                <div><label className="text-[10px] text-revelio-subtle">Coste empresa</label><div className="rounded border border-revelio-border/30 px-2 py-1 text-xs text-right font-bold text-revelio-green bg-revelio-green/5">{costeEmp > 0 ? fmtEur(costeEmp) : '—'}</div></div>
              </div>
              {costeEmp > 0 && <div className="mt-1.5 flex gap-3 text-[10px] text-revelio-subtle"><span>Convenio: {fmtN(convH)}h</span><span className="font-bold text-revelio-green">Coste/h: {costH.toFixed(2).replace('.',',')}€</span></div>}
            </div>
          )})}
        ...
        {saveError && <div className="mt-2 bg-revelio-red/10 border border-revelio-red/20 rounded-lg px-3 py-2 text-xs text-revelio-red">{saveError}</div>}
